...
def missingness_plots(df, prefix="train"):
    ...
    fn = os.path.join(PLOT_DIR, f"{prefix}_missing_bar.png")
    plt.savefig(fn, bbox_inches="tight")
    plt.close()

    plt.figure(figsize=(14,6))
    msno.matrix(df)
    fn = os.path.join(PLOT_DIR, f"{prefix}_missing_matrix.png")
    plt.savefig(fn, bbox_inches="tight")
    plt.close()

    plt.figure(figsize=(10,8))
    msno.heatmap(df)
    fn = os.path.join(PLOT_DIR, f"{prefix}_missing_heatmap.png")
    plt.savefig(fn, bbox_inches="tight")
    plt.close()
    ...
    try:
        ...
        msno.dendrogram(df)
        fn = os.path.join(PLOT_DIR, f"{prefix}_missing_dendrogram.png")
        plt.savefig(fn, bbox_inches="tight")
        plt.close()
    except Exception as e:
        print("Skipping dendrogram (error):", e)

def drop_all_rows_with_any_missing_and_censored(train_df):
    df = train_df.dropna(axis=0, how="any")
    if CENSOR_COL in df.columns:
        df = df[df[CENSOR_COL] == 0]
    print("Strategy A: rows after drop-any-missing + uncensored:", df.shape[0])
    return df

def baseline_drop_columns_and_censored(train_df):
    protected = {TARGET_COL, CENSOR_COL}
    cols_with_missing = [c for c in train_df.columns if train_df[c].isna().any() and c not in protected]
    df = train_df.drop(columns=cols_with_missing)
    if TARGET_COL in df.columns:
        df = df[df[TARGET_COL].notna()]
    if CENSOR_COL in df.columns:
        df = df[df[CENSOR_COL] == 0]
    print("Strategy B: dropped columns:", cols_with_missing)
    print("Strategy B: rows remaining (uncensored & labeled):", df.shape[0])
...
def main():
    ...
    print("Rows after drop-all-rows strategy:", df_allrows.shape[0])
    print("Rows after assignment baseline filtering:", X.shape[0])
    print("K-fold CV cMSE:", cv_score)
    print("Hold-out cMSE:", hold_score)
    print("Train cMSE (final baseline):", train_score)
