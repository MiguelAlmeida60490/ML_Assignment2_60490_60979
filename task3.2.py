...
def cv_evaluate_direct_model(model, X, y, c, kfold, labeled_indices):
    labeled_indices = np.asarray(labeled_indices)
    fold_errors = []

    for tr_loc, te_loc in kfold.split(labeled_indices):
...
def run_model_selection(X, y, c, labeled_mask, cv_splits=5,
    ...
    for deg in poly_degrees:
        for alpha in ridge_alphas:
            ...
            for tr_loc, te_loc in kf.split(labeled_indices):
                tr, te = labeled_indices[tr_loc], labeled_indices[te_loc]
                pipe.fit(X.iloc[tr], y[tr])
                preds = pipe.predict(X.iloc[te])
                fold_errors.append(error_metric(y[te], preds, c[te]))
            results.append({
                "model": "polynomial", "degree": deg, "alpha": alpha,
    ...
    for depth in [None, 4, 6]:
        for lr in [0.05, 0.1]:
            hgb = make_hgb_pipeline(max_depth=depth, learning_rate=lr)
            fold_errors = cv_evaluate_direct_model(hgb, X.iloc[labeled_indices], y[labeled_indices], c[labeled_indices], kf, np.arange(len(labeled_indices)))
            results.append({
                "model": "hgb",
            ...
            print(f"[CV] HGB depth={depth}, lr={lr}, mean={np.mean(fold_errors):.4f}")

    if include_catboost:
        cat_params = {"depth": 6, "learning_rate": 0.05}
        fold_errors = []
        feature_cols = [c for c in train_df.columns if c not in [ID_COL, TARGET_COL, CENSOR_COL]]
        cat_features = train_df[feature_cols].select_dtypes(include=['object','category']).columns.tolist()
        for tr_loc, te_loc in kf.split(labeled_indices):
            tr, te = labeled_indices[tr_loc], labeled_indices[te_loc]
            model = CatBoostRegressor(**cat_params, cat_features=cat_features, random_state=RANDOM_STATE, verbose=False)
            model.fit(train_df.loc[tr, feature_cols], y[tr])
            preds = model.predict(train_df.loc[te, feature_cols])
            fold_errors.append(error_metric(y[te], preds, c[te]))
        results.append({
            "model": "catboost",
            "params": cat_params,
        ...
        print(f"[CV] CatBoost mean={np.mean(fold_errors):.4f}")

    if include_catboost_aft:
        cat_params = {"depth": 6, "learning_rate": 0.05}
        fold_errors = []
        feature_cols = [c for c in train_df.columns if c not in [ID_COL, TARGET_COL, CENSOR_COL]]
        cat_features = train_df[feature_cols].select_dtypes(include=['object','category']).columns.tolist()
        for tr_loc, te_loc in kf.split(labeled_indices):
            ...
            model, _, _ = train_catboost_aft_regressor(cat_params, train_df.loc[tr].reset_index(drop=True))
            preds = model.predict(train_df.loc[te, feature_cols])
            fold_errors.append(error_metric(y[te], preds, c[te]))
        ...
        print(f"[CV] CatBoost AFT mean={np.mean(fold_errors):.4f}")
    ...
    summary_df = pd.DataFrame(summary_rows).sort_values(by=["mean_error","std_error"]).reset_index(drop=True)
...
def train_final_and_generate_submission(best_entry, X, y, c, X_test_df, sample_sub, out_filename, train_df):
    labeled_mask = ~pd.isna(y)
    
    if best_entry["model"] == "polynomial":
        pipe = make_polynomial_pipeline(best_entry["degree"], best_entry["alpha"])
        pipe.fit(X.loc[labeled_mask], y[labeled_mask])
        
    elif best_entry["model"] == "hgb":
        pipe = make_hgb_pipeline(**best_entry["params"])
        pipe.fit(X.loc[labeled_mask], y[labeled_mask])
        
    elif best_entry["model"] == "catboost":
        pipe, feat_cols, cat_cols = train_catboost_regressor(best_entry["params"], train_df)
        
    elif best_entry["model"] == "catboost_aft":
        pipe, feat_cols, cat_cols = train_catboost_aft_regressor(best_entry["params"], train_df)
        
    else:  
        pipe = make_knn_pipeline(best_entry["k"])
        pipe.fit(X.loc[labeled_mask], y[labeled_mask])
    
    if best_entry["model"] in ["catboost", "catboost_aft"]:
        X_train_pred = X.loc[labeled_mask, feat_cols]
        X_test_pred = X_test_df[feat_cols]
    else:
        X_train_pred = X.loc[labeled_mask].drop(columns=[ID_COL], errors="ignore")
        X_test_pred = X_test_df.drop(columns=[ID_COL], errors="ignore")
    
    train_preds = pipe.predict(X_train_pred)
    train_err = error_metric(y[labeled_mask], train_preds, c[labeled_mask])
    
    test_preds = pipe.predict(X_test_pred)
    sub = sample_sub.copy()
    target_col = sub.columns[1] if sub.shape[1] >= 2 else TARGET_COL
...
def make_hgb_pipeline(max_depth=None, learning_rate=0.1, max_iter=300):
...
def train_catboost_regressor(params, train_df):
    feature_cols = [c for c in train_df.columns if c not in [ID_COL, TARGET_COL, CENSOR_COL]]
    cat_features = train_df[feature_cols].select_dtypes(include=['object', 'category']).columns.tolist()
    ...
    labeled_mask = ~y.isna()
    model.fit(X.loc[labeled_mask], y.loc[labeled_mask])
    return model, feature_cols, cat_features

def train_catboost_aft_regressor(params, train_df):
    feature_cols = [c for c in train_df.columns if c not in [ID_COL, TARGET_COL, CENSOR_COL]]
    cat_features = train_df[feature_cols].select_dtypes(include=['object', 'category']).columns.tolist()

    X = train_df[feature_cols]
    y = train_df[TARGET_COL]
    c = train_df[CENSOR_COL]

    # Detect GPU
    try:
        from catboost import _catboost
        use_gpu = _catboost._is_gpu_available()
    except Exception:
        use_gpu = False

    if not use_gpu:
        raise RuntimeError(
            "CatBoost AFT requires GPU in this version of CatBoost. "
            "Please install a GPU-enabled version (`pip install catboost[gpu]`) "
            "or downgrade to CatBoost 1.1 for CPU AFT."
        )

    mask = ~y.isna()

    model = CatBoostRegressor(
        loss_function="AFT:distribution=loglogistic",
        task_type="GPU",
        devices='0', 
...
def main():
    ...
    X_train_pre, X_test_pre, _ = preprocess_features(train_df, test_df)
    ...
    results, summary_df = run_model_selection(
        ...
        include_catboost_aft=True,
        train_df=train_df
    )

    best_model = select_best_model_from_summary(results)
    print("Selected best model from CV:", best_model)

    submission_path = get_next_submission_filename(
        task_num=3,
        base_name=OUT_SUBMISSION
    )
    ...
    try:
        mask = ~pd.isna(y_all)
        if best_model["model"] in ["catboost", "catboost_aft"]:
            feat_cols = [c for c in train_df.columns if c not in [ID_COL, TARGET_COL, CENSOR_COL]]
            plot_y_vs_yhat(y_all[mask], final_pipeline.predict(X_all.loc[mask, feat_cols]), 
                            os.path.join(PLOT_DIR, "y_vs_yhat_final.png"))
        else:
            plot_y_vs_yhat(y_all[mask], final_pipeline.predict(X_all.loc[mask].drop(columns=[ID_COL], errors="ignore")), 
                            os.path.join(PLOT_DIR, "y_vs_yhat_final.png"))
    except Exception as e:
    ...
    print("Final training rows:", labeled_mask.sum())
    print("Final feature count:", X_all.shape[1])
    print("Train cMSE:", train_err)
    print("Saved CV summary (top rows):\n", summary_df.head())
